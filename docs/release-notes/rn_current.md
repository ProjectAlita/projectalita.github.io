...
- **Datasources deprecated**: The **Datasources** entity is deprecated. Your existing datasources and datasets remain available in this release so you can re-index and reconfigure them using the new **Index data** tool available in most Toolkits (e.g., GitHub, Confluence, Bitbucket, ADO Repos, Jira, etc.). **Note**: Creating new datasources or datasets is no longer supported.
